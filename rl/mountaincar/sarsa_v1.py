...
class Sarsa(ExperienceReplay):
    ...
    def __init__(self, network_parameter, max_memory=100, discount=.99, n_steps=1, alpha=.1, num_actions=3, epsilon=.1):
        ...
        for i in range(network_parameter.n_hidden_layer):
            ...
            self.model.add(Dropout(rate=0))
        self.model.add(Dense(network_parameter.num_actions))
        # self.model.compile(sgd(lr=0.01, momentum=0.99), loss='mse')
        self.model.compile(sgd(lr=0.01, momentum=0.99, nesterov=True), loss='mse')
    ...
    def cumulative_rewards(self, state, next_time_steps, game_over):
        ...
        targets = self.model.predict(state_t)[0]
        inputs = state_t
        # sum all reward
        ...
        if game_over is False:
            # print("calling predict next state")
        ...
        # cumulative_rewards = targets[action_t] + self.alpha * (cumulative_rewards - targets[action_t])
    ...
    def get_batch(self, batch_size=10):
        ...
        for i, idx in enumerate(np.random.randint(0, len_memory, size=inputs.shape[0])):
            ...
            if game_over:  # if game_over is True
                ...
                next_time_steps = self.memory[idx: idx + self.n_steps + 1 + 1]
                ...
                # print("action updated", targets[i, action_t])
