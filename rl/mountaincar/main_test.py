"""
    There is one issue with using neural network as Q approximator. The transitions are very correlated.
    This reduce the overall variance of the transition
"""
...
if __name__ == "__main__":
    ...
    experience_replay = Sarsa(network_parameter, max_memory=max_memory, n_steps=number_of_steps, num_actions=num_actions, epsilon=epsilon)

    experience_replay.model.load_weights("model.h5")
    ...
    for e in range(epoch):
        ...
        while (not game_over):
            ...
            # if (number_of_steps + 1) > step:
            #    continue

            #inputs, targets = experience_replay.get_batch(batch_size=50)
            # update model
            step_loss = 0  # experience_replay.train_on_batch(inputs, targets)
